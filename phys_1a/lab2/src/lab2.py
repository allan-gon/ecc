...
def generate_data():
    ...
    for i in range(6):
        with open("./data/lab2_data.csv", "a") as file:
            low_bound = ceil(
                (MEW_K * (MASS_1 + (i * 100))) + randint(-2, 2)
            )  # whatever the estimate is, add noise to it, and since weights are no smaller than 1 gram round up a gram always
            file.write(
                f"{MASS_1 + (i * 100)},{low_bound},{low_bound + 1},{low_bound + .5}\n"
            )  # save the masses
...
def graph():
    ...
    fig = px.scatter(
        df, x="mass_1(g)", y="mass_2_best(g)", error_y="error", trendline="ols"
    )
    fig.show()
...
if __name__ == "__main__":
    ...
    slope_min, slope_max = round((150 - 332) / (411.4 - 911.4), 4), round(
        (149 - 333) / (411.4 - 911.4), 4
    )
